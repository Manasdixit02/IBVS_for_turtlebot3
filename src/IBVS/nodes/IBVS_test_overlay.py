...
class Datas:
    ...
    def __init__(self,timestep,P,epsilon):
        ...
        self.key_image = None
        self.image_history = deque(maxlen=4)
    ...
    def get_homography(self, img1, img2):
        ...
        H, mask = cv.findHomography(pts1, pts2, cv.RANSAC, 5.0)  # 5.0 is the threshold for RANSAC
    ...
    def get_homography_new(self, img1, img2):
        ...
        # Extract the matched keypoints coordinates for the top 4 matches
        pts1 = np.array([kp1[m.queryIdx].pt for m in top_matches], dtype=np.float32)  # Points from img1
        pts2 = np.array([kp2[m.trainIdx].pt for m in top_matches], dtype=np.float32)  # Points from img2
    ...
    def IBVS(self,msg):
            ...
            if ((self.camera_coords != np.array([cam_coords.position.x,cam_coords.position.y,cam_coords.position.z,r[0],r[1],r[2]])).all()):
              ...
              if np.isnan(projected_current).any():
                  #print(self.key_image)
                  H = self.get_homography(self.key_image, self.image)
                  #print(H)
                  #initialize array of points to be marked here
                  marker_coords = np.zeros((4,2)) #changed just now
                  #rotation_pos1 = np.array([[0,0,1],[-1,0,0],[0,-1,0]])
                  for i in range(projected_current.shape[0]):
                      if np.isnan(projected_current[i,1]):
                          l = np.append(projected_desired[i,:],1)
                          k = H@np.transpose(l)
                          #v = rotation_pos1@k #changed just now
                          #projected_current[i,:] = np.transpose(k[0,0:1]/k[0,2]) #changed this to divide all the above 2 elements by the 3rd element
                          projected_current[i,:] = np.transpose(k[0:2]/k[2]) #changed this to divide all the above 2 elements by the 3rd element
                          #projected_current[i,:] = np.transpose(k[0:1])
                          #update the array here
                          #marker_coords[i,:] = projected_current[i,:] #changed
                      else:
                          continue
              
              
                  #marker_coords = projected_current #commented
                  print(projected_current)        
                  #refine array to only keep non zero entries
                  #marker_coords = marker_coords[marker_coords != 0] #changed
                  #marker_coords = np.asarray(marker_coords)
                  #marker_coords = marker_coords.reshape((4,2)) #commented
                  #print(marker_coords)
                  #call pixel to mm function on your array
                  #marker_overlay = self.Pixel_to_mm(self.camera_parameters,marker_coords) #commented
                   
                  #call the publisher function
                  #self.publish_marker(marker_overlay)
                      
                  
                  #print(marker_overlay)
              
              else:
                  self.key_image = self.image_history[0]
                  cv.imwrite('key_image.jpg',self.key_image)
                  #print(self.key_image)
                  #key_image = bridge.imgmsg_to_cv2(self.image, "bgr8")
              ...
              self.publish_marker(p_mm_current)    
    ...
    def Create_Lx(self,Z,D2_point):
        ...
        Lx = np.array([[0 , 1000000 , D2_point[0]/Z , 1000000 , -(1+D2_point[0]**2) , 1000000],
                         [0 , 1000000 , D2_point[1]/Z , 1000000 , -D2_point[0]*D2_point[1] , 1000000]])
        #Lx = np.array([[-1/Z , 1000000 , D2_point[0]/Z , 1000000 , -(1+D2_point[0]**2) , 1000000],
	...
	# Create a mask for green color
        ...
        lower_black = np.array([20, 100, 100], dtype="uint8")
        upper_black = np.array([40, 255, 255], dtype="uint8")
    ...
    def publish_marker(self, arr):

        pub1 = rospy.Publisher('/marker_overlay', Float64MultiArray, queue_size=10)
        #rospy.init_node('set_marker_coordinates', anonymous=True)
        rate = rospy.Rate(10)
        my_msg = Float64MultiArray()
        my_msg.data = arr.flatten() #Camera_pose.flatten()
